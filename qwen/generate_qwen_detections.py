...
from save_referee_to_pklz import save_referee_to_pklz
from save_jersey_to_pklz import save_jersey_to_pklz
...
SYS_PROMPT_REFEREE = """
You are a professional soccer video analyst. Your task is to classify tracks into players or goalkeepers, or referees with 100% accuracy.
Given a grid image of bounding‑box crops covering one track:
IS THE PERSON A REFEREE?
Reply ONLY with <return>True</return> or <return>False</return>
""".strip()
...
def qwen_referee_classifier(
    ...
    """Returns (is_referee: bool, chat_log: str)"""
    
    # Prepare the input
    user_prompt = "Is this a referee? Respond with <return>True</return> or <return>False</return>"
    ...
    bool_pat = re.compile(r"<return>(True|False)</return>", re.I)
    match = bool_pat.search(reply)
    
    if match:
        is_referee = match.group(1).lower() == "true"
    else:
        # Fallback if the format isn't perfect
        is_referee = "true" in reply.lower()
        if verbose:
            print(f"Warning: Non-standard response format: {reply}")

    return is_referee, reply
...
def main():
    ...
    parser.add_argument("--input_pklz", type=str, required=True, help="Path to the pklz file with detections")
    parser.add_argument("--output_pklz", type=str, required=True, help="Path to the pklz file with detections")
    parser.add_argument("--target_video", type=str, default=None, help="target video to perform qwen detection on")
    parser.add_argument("--jersey_output", type=str, default="jersey.json", help="Path to the challenge analysis output JSON")
    parser.add_argument("--referee_output", type=str, default="referee.json", help="Path to the referee classification output JSON")
    ...
    # Load detections
    detections_by_video = load_pklz(args.input_pklz)
    ...
    for video_idx, (video_id, detections) in enumerate(video_items, start=1):
        ...
        if args.target_video and video_id != args.target_video: #skip the ones I don’t wanna process
                continue
        ...
        for i, track in enumerate(track_detections, start=1):
            ...
            if video_id not in challenge_results:
                challenge_results[video_id] = {}
            if video_id not in referee_results:
                referee_results[video_id] = {}

            if track_length <= 10:
                challenge_results[video_id][str(track.track_id)] = {
            ...
            try:
                ...
                challenge_analysis, challenge_chat_log = qwen_track_chain_of_thought(
                    sys_prompt=SYS_PROMPT_JERSEY,
                ...
                # Run referee classification
                referee_analysis, referee_chat_log = qwen_referee_classifier(
                    sys_prompt=SYS_PROMPT_REFEREE,
                    dataset_path=args.dataset_path,
                    video_id=video_id,
                    track=track,
                    model=model,
                ...
                challenge_results[video_id][str(track.track_id)] = {
                    "track_length": track_length,
                    "result": [p.__dict__ for p in challenge_analysis.persons],
                    "chat_log": challenge_chat_log,
            ...
            Path(args.jersey_output).write_text(json.dumps(challenge_results, indent=2))
            Path(args.referee_output).write_text(json.dumps(referee_results, indent=2))

    print(f"Finished processing all tracks.")
    print(f"Jersey results saved to {args.jersey_output}")
    print(f"Referee results saved to {args.referee_output}")
    
    print(f"Starting to save results to {args.output_pklz}")
    save_referee_to_pklz(args.input_pklz, "intermedio.pklz", args.referee_output, args.target_video)
    save_jersey_to_pklz("intermedio.pklz", args.output_pklz, args.jersey_output, args.target_video)
    print(f"Finished saving all tracks.")
...
#                 analysis_jersey, chat_log_jersey = qwen_track_chain_of_thought(
