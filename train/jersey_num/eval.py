...
def calculate_accuracy_and_counts(gt, pred):
    class_correct = [0] * 100
    class_total = [0] * 100
...
def plot_heatmap(accuracies, counts, dataset_name):
...
def save_misclassified_images(x_batch, gt_batch, pred_labels, batch_index, folder="misclassified_images"):
...
def annotate_image(image_path, gt, pred):
...
def calculate_overall_accuracy(total_gt, total_pred):
...
def eval(train_datasetpath, valid_datasetpath, test_datasetpath, model_path, model_name = "convnextv2_nano.fcmae_ft_in1k"):    
    device = "cuda"
    ...
    # jerseynum_train_dataset = JerseyNumberDataset(dataset_folder=train_datasetpath, transform=inference_transform, include_number_not_seen=True)
