...
class SoccerNetGameState(TrackingDataset):
    ...
    def process_trackeval_results(self, results, dataset_config, eval_config):
        ...
        # Extract the GS-HOTA metric
        gs_hota_metric = combined_results['GS-HOTA']['GS-HOTA']
        
         # Get the currently active run
        active_run = mlflow.active_run()
        if active_run:
            # Log the GS-HOTA metric
            mlflow.log_metric("GS-HOTA", gs_hota_metric)
        
            # Log the dataset configuration parameters
            mlflow.log_params({
                "EVAL_SPACE": dataset_config['EVAL_SPACE'],
                "USE_JERSEY_NUMBERS": dataset_config['USE_JERSEY_NUMBERS'],
                "USE_TEAMS": dataset_config['USE_TEAMS'],
                "USE_ROLES": dataset_config['USE_ROLES'],
                "EVAL_DIST_TOL": dataset_config['EVAL_DIST_TOL']
            })
            mlflow.end_run()
        else:
            print("No active MLflow run found. Please start a run before logging metrics and parameters.")
        # Log the GS-HOTA metric and configuration details to MLflow
      #  with mlflow.start_run(experiment_id="833431500785556588"):  # Replace with your experiment ID or name
            # Log the GS-HOTA metric
       #     mlflow.log_metric("GS-HOTA", gs_hota_metric)
